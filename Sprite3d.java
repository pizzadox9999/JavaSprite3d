//ported by pizza_dox_9999 (http://github.com/pizza_dox_9999)
//c++ sourcecode by Hapax (http://github.com/Hapaxia)
...
public class Sprite3d extends Sprite implements Drawable, Transformable{
  ...
  private float[] m_compactTransformMatrix;
  ...
  public Sprite3d(){
    ...
    m_points=new ArrayList<Vector3f>(4);Collections.fill(m_points, Vector3f.ZERO);    
    m_transformedPoints=new ArrayList<Vector2f>(4);Collections.fill(m_transformedPoints, Vector2f.ZERO);
    m_origin=new Vector3f(0, 0, 0);
    m_vertices=new ArrayList<Vertex>(4);Collections.fill(m_vertices, new Vertex(Vector2f.ZERO));
    m_isBackFacing=false;
    m_compactTransformMatrix=new float[5];
  ...
  public void setBackFlipEnabled(){
    this.setBackFlipEnabled(true);
  }
  ...
  public float getMostExtremeAngle() {
    float pitch = (float)Math.abs(m_pitch);
    if (pitch > 90.f)
      pitch = 180.f - pitch;
  ...
  public void setDynamicSubdivisionEnabled(){
    this.setDynamicSubdivisionEnabled(true);
  }
  public void setDynamicSubdivisionEnabled(boolean enabled){
    m_useDynamicSubdivision = enabled;
  }
  public void setDynamicSubdivisionRange(int maximum){
    this.setDynamicSubdivisionRange(maximum, 0);
  }
  ...
  private void updateTransformedPoints(){
    ...
    *******************************************************/
    ...
    m_compactTransformMatrix[0]=cosYaw;
    m_compactTransformMatrix[1]=sinYaw;
    m_compactTransformMatrix[2]=sinPitch * sinYaw;
    m_compactTransformMatrix[3]=cosPitch;
    m_compactTransformMatrix[4]=-sinPitch * cosYaw;
  ...
  private void updateVertices() {
    ...
    for (int v = 0; v < m_vertices.size(); ++v){
      ...
      // update vertex
      m_vertices.set(v, new Vertex(m_transformedPoints.get(pointIndex),
                                   m_vertices.get(v).color,
                                   new Vector2f(
                                                (m_points.get(texturePointIndex).x * (m_size.x < 0 ? -1 : 1)) + currentTextureOffset.x,
                                                (m_points.get(texturePointIndex).y * (m_size.y < 0 ? -1 : 1)) + currentTextureOffset.y)
                                  )
  ...
  private int getPointIndexForVertexIndex(int vertexIndex, boolean invertPointX){
    int numberOfPointsPerDimension = m_subdividedMeshDensity + 2;
  ...
  private int getNumberOfVerticesNeededForCurrentSubdividedMeshDensity(){
  ...
  private float linearInterpolation(float from, float to, float alpha){
    return from * (1.f - alpha) + to * alpha;
  }
  
  private float mod(float numerator, float denominator){
  ...
  private Vector2i abs(Vector2i vector){
